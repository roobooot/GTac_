...
String readString;
int pos_1;
const PROGMEM int pos_1_roof = 160;
const PROGMEM int pos_1_floor = 1;
int pos_2;
const PROGMEM int pos_2_roof = 100;
const PROGMEM int pos_2_floor = 180;
int pos_3;
const PROGMEM int pos_3_roof = 180;
const PROGMEM int pos_3_floor = 15;
int pos_4;
const PROGMEM int pos_4_roof = 1;
const PROGMEM int pos_4_floor = 180;
int pos_5;
const PROGMEM int pos_5_roof = 180;
const PROGMEM int pos_5_floor = 15;
int pos_6;
const PROGMEM int pos_6_roof = 180;
const PROGMEM int pos_6_floor = 5;
...
#define Servo_1 8 //thumb flexion  8-
#define Servo_2 12 //thumb abduction  12+
#define Servo_3 6 //midlle 6+
#define Servo_4 7 //index 7+
#define Servo_5 10 //little 10+
#define Servo_6 9 //ring 9-
...
#define NUM_Data 292
#define NUM_MAG 45
#define NUM_RES 240 //piezoresistive sensing
...
void loop() {
  ...
  if (SENT == true) {
    ...
    data_to_send[285] = myservo_1.read();
    data_to_send[286] = myservo_2.read();
  ...
  if (currentTime - previousTime > timeIntervalData) {
    ...
    //print data
    print_data(data_to_send, NUM_Data);
//    send_data(data_to_send, NUM_Data);
//    print_all_data(data_to_send);
    SENT = true;
  }
...
void move_fingers() {
  ...
  if (readString.length() > 0) {
    String motor = readString[0];
    int n = motor.toInt();  //convert readString into a number
    Serial.print("motor:");
    Serial.println(n);  //so you can see the captured string
    //  Serial.println("initiate position");
    //  pinch(n);
    String deg = "";
    for (int i = 1; i < sizeof(readString); i++) {
      String d = readString[i];
      deg = deg + d;
    }
    int degree = deg.toInt();
    Serial.print("degree to move:");
...
void read_all_data() {
  ...
  for (uint8_t col = 0; col < 12; col++) {
    ...
    read_resistive_data(&data_to_send[45 + 20 * col], &data_to_send[45 + 20 * col + 1], &data_to_send[45 + 20 * col + 2], &data_to_send[45 + 20 * col + 3],
                        &data_to_send[45 + 20 * col + 4], &data_to_send[45 + 20 * col + 5], &data_to_send[45 + 20 * col + 6], &data_to_send[45 + 20 * col + 7],
                        &data_to_send[45 + 20 * col + 8], &data_to_send[45 + 20 * col + 9], &data_to_send[45 + 20 * col + 10], &data_to_send[45 + 20 * col + 11],
                        &data_to_send[45 + 20 * col + 12], &data_to_send[45 + 20 * col + 13], &data_to_send[45 + 20 * col + 14], &data_to_send[45 + 20 * col + 15],
                        &data_to_send[45 + 20 * col + 16], &data_to_send[45 + 20 * col + 17], &data_to_send[45 + 20 * col + 18], &data_to_send[45 + 20 * col + 19]);

    //I2C data
    if (col == 11) {
      read_mag_finger(bus1, &data_to_send[0], &data_to_send[1], &data_to_send[2],
                      &data_to_send[3], &data_to_send[4], &data_to_send[5],
                      &data_to_send[6], &data_to_send[7], &data_to_send[8]);
      read_mag_finger(bus2, &data_to_send[9], &data_to_send[10], &data_to_send[11],
                      &data_to_send[12], &data_to_send[13], &data_to_send[14],
                      &data_to_send[15], &data_to_send[16], &data_to_send[17]);
...
void read_resistive_data(int16_t* mat1, int16_t* mat2, int16_t* mat3, int16_t* mat4,
                         ...
                         int16_t* mat9, int16_t* mat10, int16_t* mat11, int16_t* mat12,
                         int16_t* mat13, int16_t* mat14, int16_t* mat15, int16_t* mat16,
                         int16_t* mat17, int16_t* mat18, int16_t* mat19, int16_t* mat20)
{
  switch_SP4T(LOW, LOW);
  *mat1 = analogRead(A0);
  *mat5 = analogRead(A1);
...
void read_mag_finger(byte bus, int16_t* mag_x, int16_t* mag_y, int16_t* mag_z,
                     int16_t* mag_x_2, int16_t* mag_y_2, int16_t* mag_z_2,
                     int16_t* mag_x_3, int16_t* mag_y_3, int16_t* mag_z_3) {
  tcaselect(bus);
