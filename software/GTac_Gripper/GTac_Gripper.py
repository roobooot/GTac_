...
class gtac_gripper:
    ...
    def read_GTac(self, time_stamp=None, verbose=False, to_avg=True, sens_aft_ct=True):
        ...
        if to_avg:
            ...
            data_raw[-1 - gtac_config.MOTOR_NUM:-1] = data_raw[-1 - gtac_config.MOTOR_NUM:-1] - self.motors_init  # init motor angles for gripper
        ...
        if verbose:
            ...
            print('GTac Gripper data After Pre-process: {}'.format(self.data_gtac))
    ...
    def run(self, q_gtac, q_motors_todo, q_motors_todo2, datapoints=5000, avg_gtac=True, sens_aft_ct=True):
        ...
        while i < datapoints:
            ...
            self.read_GTac(to_avg=avg_gtac, verbose=False, sens_aft_ct=sens_aft_ct, time_stamp=time_stamp)
...
def controller(q_gtac, q_motors, q_ur10_tool_move, data_points=1000):
    ...
    while i < data_points:
        ...
        motors = [0, 0, 0, 0, 0, 0]
        ur_tool = [0, 0, 0, 0, 0, 0]
        sum, _, _ = GTac_Data.gtac_data.find_FAI_sum_press_loc(data_gtac__c, finger, sec)
        sum_ur, _, _ = GTac_Data.gtac_data.find_FAI_sum_press_loc(data_gtac__c, finger_ur, sec_ur)
        SAII_vec, SAII_scale = GTac_Data.gtac_data.find_SAII(data_gtac__c, finger, sec)
        SAII_vec_ur, SAII_scale_ur = GTac_Data.gtac_data.find_SAII(data_gtac__c, finger_ur, sec_ur)
...
def flex_finger(f, g_sum_f_list, motors, g_th, incremt=5):
    if g_sum_f_list[f] < g_th:
        motors[gtac_config.MOTOR_MAP[f][0]] = incremt
    return motors


def find_f_sum(data_gtac):
    g_fc_array = np.zeros([5, 3])
    g_sum_f_list = [0, 0, 0, 0, 0]
    for f in range(5):
        g_sum_f = 0
        for s in range(3):
            _, g_fc = GTac_Data.gtac_data.find_sec_data(data_gtac, f, s, a=0.3)
            g_fc_array[f, s] = g_fc
            g_sum_f += g_fc
        g_sum_f_list[f] = round(g_sum_f, 1)
    return g_sum_f_list, g_fc_array

def find_f_sum_exclude_palm(data_gtac):
    g_fc_array = np.zeros([5, 3])
    g_sum_f_list = [0, 0, 0, 0, 0]
    for f in range(5):
        g_sum_f = 0
...
def tactile_gsp(NOF, g_fc_array, motors, incremt=8, g_th=300, force_allowance = 0.1):
    flex_abd = 0  # only flexion is involved in this function
    motors_copy = copy.copy(motors)
    motor_index = gtac_config.MOTOR_MAP[NOF][flex_abd] - 1

    f_sum_list = [0, 0, 0, 0, 0]
    for f in range(5):
        g_sum_f = 0
        for s in [1, 2]:  # only involved the tactile feedback from the fingers excluding palm
            g_sum_f += g_fc_array[f, s]
        f_sum_list[f] = round(g_sum_f, 1)

    if f_sum_list[gp_return_sensor_no(NOF)] < g_th:
        motors_copy[motor_index] = incremt
    if f_sum_list[gp_return_sensor_no(NOF)] > (1 + force_allowance) * g_th:
        motors_copy[motor_index] = -incremt
    return motors_copy

def tactile_grasping_threshold_controller(q_gtac, q_motors, data_points=1000, NOF_to_move = [0, 1, 2, 3]):
    # this threshold controller is used for tactile grasping.
    i = 0
    incremt = 8  # motor increment each time step.
    motor_speed_inv = 25  # to adjust the grasping speed
    flex_abd = 0  # flex: 0; abd: 1
    g_th = 200
    force_allowance = 0.5
    while i < data_points:
        i += 1
        ...
        f_sum_list, g_fc_array = find_f_sum(data_gtac=data_gtac)
        motor_angles = data_gtac[-2 - gtac_config.MOTOR_NUM:-2]
        for NOF in NOF_to_move:
            print('{}/{}, NOF-{}: {}'.format(i, data_points, NOF, f_sum_list[gp_return_sensor_no(NOF)]))
            print('Section wise: {}'.format(g_fc_array[gp_return_sensor_no(NOF), :]))
            print('Motor angles: {}'.format(motor_angles))
            motors = tactile_gsp(NOF=NOF, g_fc_array=g_fc_array, motors=motors,
                                 incremt=incremt, g_th=g_th, force_allowance=force_allowance)
        # motor command execution
        if i % motor_speed_inv == 0 and i > 0:
            if not q_motors.full():
                q_motors.put(motors)

def in_hand_manip_rolling_ball(q_gtac, q_motors, data_points=1000, NOF_to_move = [0, 1, 2, 3]):
    i = 0
    incremt = 8
    motor_speed_inv = 20
    g_th = 250
    NOF_to_abd = NOF_to_move
    abd_index = 0  # to hold the object for a while after grasped states
    abd_index_gap = 100  # the time gap to hold the object before rolling command
    ret_index = 0  #
    return_gap = 100  # the cycles gap to return the fingers after rolling the object
    to_return = False
    return_incremt = 0
    rolling_incremt = 200
    rolling_return = 0
    while i < data_points:
        ...
        for NOF in NOF_to_move:
            # print('{}/{}, NOF-{}: {}'.format(i, data_points, NOF, f_sum_list[gp_return_sensor_no(NOF)]))
            print('{}/{}, NOF-{}: Section wise: {}'.format(i, data_points, NOF, g_fc_array[gp_return_sensor_no(NOF), :]))
            print('Motor angles: {}'.format(motor_angles))
            if not to_return:
                motors = tactile_gsp(NOF=NOF, g_fc_array=g_fc_array, motors=motors,
                                 incremt=incremt, g_th=g_th, force_allowance=0.3)

            # motor_index_flex = gtac_config.MOTOR_MAP[NOF][0] - 1
            # motor_index_abd = gtac_config.MOTOR_MAP[NOF][1] - 1
            # if dir == 1:
            #     if motor_angles[motor_index_abd] < abs(abd_range):
            #         if motors[motor_index_flex] == 0:
            #             motors[motor_index_abd] = -incremt
            # if dir == -1:
            #     if motor_angles[motor_index_abd] > -abs(abd_range):
            #         if motors[motor_index_flex] == 0:
            #             motors[motor_index_abd] = incremt
            # if NOF in NOF_to_abd:
            #     motors = tactile_abd(NOF=NOF, motor_angles=motor_angles, motors=motors, abd_range=20, direction=1)
        hold_to_abd = 0  # to rotate the finger (NOF_to_abd) at the same time, we check the number of finger in grasped states.
        for nof_abd in NOF_to_abd:  # check the grasped states of the fingers to abd
            motor_index_flex = gtac_config.MOTOR_MAP[nof_abd][0] - 1
            if motors[motor_index_flex] == 0:
                hold_to_abd += 1
        if hold_to_abd == len(NOF_to_abd) and not to_return:
            abd_index += 1
        if hold_to_abd == len(NOF_to_abd) and abd_index > abd_index_gap:  # the number of fingers in grasped states == the NOF_to_abd, so give the rotation command.
            print('to roll the object, abd_index: {}'.format(abd_index))
            for nof_abd in NOF_to_abd:
                motor_index_abd = gtac_config.MOTOR_MAP[nof_abd][1] - 1
                motor_index_flex = gtac_config.MOTOR_MAP[nof_abd][0] - 1
                motors[motor_index_abd] = rolling_incremt
                motors[motor_index_flex] = rolling_return
                to_return = True
            if not q_motors.full():  # send the command immediately
                q_motors.put(motors)
            abd_index = 0

        print('abd_index: {}; to_return: {}; ret_index: {};'.format(abd_index, to_return, ret_index))
        print('hold_to_abd: {}; NOF_to_abd: {};'.format(hold_to_abd, NOF_to_abd))
        ...
        if ret_index > return_gap:  #
            motors = [0, 0, 0, 0, 0, 0, 0, 0]
            for nof_abd in NOF_to_abd:
                motor_index_flex = gtac_config.MOTOR_MAP[nof_abd][0] - 1
                motor_index_abd = gtac_config.MOTOR_MAP[nof_abd][1] - 1
                motors[motor_index_abd] = -rolling_incremt
                motors[motor_index_flex] = -return_incremt
            if not q_motors.full():
                q_motors.put(motors)
            ret_index = 0
            to_return = False

def in_hand_manip_translation(q_gtac, q_motors, data_points=1000, NOF_to_move = [0, 1, 2, 3]):
    i = 0
    incremt = 8
    motor_speed_inv = 15
    g_th = 200
    g_th_translate = 200
    g_th_translate_tor = 0.5
    # NOF_to_abd = NOF_to_move
    NOF_to_abd = [2, 3]
    grasped_index = 0  # to hold the object for a while after grasped states
    grasped_index_gap = 100  # the time gap to hold the object before translation command
    ret_index = 0  # to hold the fingers for a while
    return_gap = 100  # the cycles gap to return the fingers after rolling the object
    to_return = False
    return_incremt = 0
    rolling_incremt = 200
    rolling_return = 0
    grasped = False
    translate_towards = 0  # 0: left; 1: right
    angle_limit = 60
    while i < data_points:
        i += 1
        data_gtac = q_gtac.get(timeout=1)
        q_gtac.task_done()
        motors = [0, 0, 0, 0, 0, 0, 0, 0]
        f_sum_list, g_fc_array = find_f_sum_exclude_palm(data_gtac=data_gtac)
        motor_angles = data_gtac[-2 - gtac_config.MOTOR_NUM:-2]
        for NOF in NOF_to_move:
            # print('{}/{}, NOF-{}: {}'.format(i, data_points, NOF, f_sum_list[gp_return_sensor_no(NOF)]))
            print('{}/{}, NOF-{}: Section wise: {}'.format(i, data_points, NOF, g_fc_array[gp_return_sensor_no(NOF), :]))
            ...
            if not grasped:
                motors = tactile_gsp(NOF=NOF, g_fc_array=g_fc_array, motors=motors,
                                 incremt=incremt, g_th=g_th, force_allowance=2)

        hold_to_abd = 0  # to rotate the finger (NOF_to_abd) at the same time, we check the number of finger in grasped states.
        for nof_abd in NOF_to_abd:  # check the grasped states of the fingers to abd
            motor_index_flex = gtac_config.MOTOR_MAP[nof_abd][0] - 1
            if motors[motor_index_flex] == 0:
                hold_to_abd += 1  # check if all the fingers are in grasped state
        if hold_to_abd == len(NOF_to_abd) and not grasped:
            grasped_index += 1
        if grasped_index > grasped_index_gap:  # the number of fingers in grasped states == the NOF_to_abd, so give the rotation command.
            grasped = True
        g_sum_both = f_sum_list[gp_return_sensor_no(NOF_to_abd[0])] + f_sum_list[gp_return_sensor_no(NOF_to_abd[1])]
        print('grasped_index: {}; grasped: {}; ret_index: {};'.format(grasped_index, grasped, ret_index))
        print('hold_to_abd: {}; NOF_to_abd: {}; g_sum_both: {}'.format(hold_to_abd, NOF_to_abd, g_sum_both))
        if grasped and translate_towards == 0: # to the one side
            if g_sum_both > g_th_translate:
                if g_sum_both > g_th_translate * (1 + g_th_translate_tor):
                    motor_index_flex = gtac_config.MOTOR_MAP[NOF_to_abd[1]][0] - 1
                    motors[motor_index_flex] = -incremt
                else:
                    motor_index_flex = gtac_config.MOTOR_MAP[NOF_to_abd[0]][0] - 1
                    motors[motor_index_flex] = incremt
                    motor_index_flex = gtac_config.MOTOR_MAP[NOF_to_abd[1]][0] - 1
                    motors[motor_index_flex] = -incremt
            else:
                ...
                motor_index_flex = gtac_config.MOTOR_MAP[NOF_to_abd[1]][0] - 1
                motors[motor_index_flex] = incremt
            if abs(motor_angles[NOF_to_abd[0]]) > angle_limit:
                translate_towards = 1

        if grasped and translate_towards == 1: # to the another side
            if g_sum_both > g_th_translate:
                if g_sum_both > g_th_translate * (1 + g_th_translate_tor):
                    motor_index_flex = gtac_config.MOTOR_MAP[NOF_to_abd[0]][0] - 1
                    motors[motor_index_flex] = -incremt
                ...
                motor_index_flex = gtac_config.MOTOR_MAP[NOF_to_abd[1]][0] - 1
                motors[motor_index_flex] = incremt
            if abs(motor_angles[NOF_to_abd[1]]) > angle_limit:
                translate_towards = 0
        # motor command execution
        if i % motor_speed_inv == 0 and i > 0:
            if not q_motors.full():
...
def tactile_abd(NOF, motor_angles, motors, abd_range=20, incremt=8, direction=1):
...
def _save_GTac(q_gtac_save, q_ECS_result_save=None, data_points=5000, save_path='data/', remarks='test'):
    ...
    while i < data_points - 10:
        ...
        if len(data_gtac) == len(gtac_config.COLUMNS_RAW_FINGER_DATA):
    ...
    df_RAW = pd.DataFrame(columns=gtac_config.COLUMNS_RAW_FINGER_DATA)
    for i, d in enumerate(dt_list):
        # dummy = q_gtac_save.get(timeout=1)
        # q_gtac_save.task_done()
        # print('length of frame to save: {}'.format(len(d)))
        # print(d)
        if len(d) == len(gtac_config.COLUMNS_RAW_FINGER_DATA):
...
def multi_clf_predict(clf, data_gtac_285):
    ...
    if isinstance(clf, ML_model_type):
        ...
        data = GTac_Data.gtac_data_analysis.reshape_for_CNN(data_gtac_285)
...
def ECS_recognition(q_gtac, q_ECS_result, q_ECS_result_handover, q_ECS_result_save, data_points=5000, dq_gsp_state=None,
                    ECS_clf=None):
    ...
    if dq_gsp_state is not None:
        while i < data_points:
            ...
            if len(dt_list) == avg_size and to_avg_:
                gtac_avg = np.mean(dt_list[-int(avg_size / 3):], axis=0,
                                   dtype=int)  # calculate avg value by the second half
            ...
            if not to_avg_ and dq_gsp_state[-1] == True:
                ...
                ecs_result_save = [result,
                                   1]  # to mark te timing of starting ECS recognition when object being grasped.
            else:
                result = 'none'
                ecs_result_save = [result,
                                   0]  # # to mark te timing of starting ECS recognition when object being grasped.
...
def grasping_and_handover(q_ur10, dq_ur10_cmd_exc, data_points=5000, dq_g_state=None, dq_gsp_cmd=None,
                          dq_stop_sign=None):
    ...
    while i < data_points:
        ...
        if phase == 1 and phase_done[1] == False and dq_g_state[-1] == False and dq_ur10_cmd_exc[
            -1] == ur_send_exc:  # start to grasp
            print('phase 1-0')
            time.sleep(3)
        ...
        if phase == 2 and phase_done[2] == False and dq_g_state[
            ...
            if not q_ur10.full() and dq_ur10_cmd_exc[-1] == ur_send_exc:
                ...
                # time.sleep(3)
        if phase == 2 and phase_done[2] == False and dq_g_state[
            -1] == True and ur_send_exc == 3 + last_ur_send_exc:  # lift and moving
            print('phase 2-2')
            print('dq_ur10_cmd_exc[-1]: {}; ur_send_exc: {}'.format(dq_ur10_cmd_exc[-1], ur_send_exc))
            if not q_ur10.full() and dq_ur10_cmd_exc[-1] == ur_send_exc:
...
def main(data_points=1000, serial_port='/dev/ttyACM0', to_save=False, remark='test', ECS_plot=0, NOF_to_move = [0, 1, 2, 3], init_pos=None, init_mt=None):
    ...
    publisher_thread = threading.Thread(target=queue_publisher,
                                        ...
                                        args=(
                                            q_gtac, q_gtac_tosave, q_gtac_mornitor, q_gtac_ECS, data_points,
                                            dq_stop_sign))
    ...
    controller_thread = threading.Thread(target=tactile_grasping_threshold_controller,
                                         name='thread-controller',
                                         args=(q_gtac_mornitor, q_motors_todo, data_points, NOF_to_move))
    save_gtac_thread = threading.Thread(target=_save_GTac,
                                        name='thread-save',
                                        args=(q_gtac_tosave, None, data_points, 'data/', remark))
