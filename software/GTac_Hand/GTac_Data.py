...
class gtac_data:
    ...
    def plot_all(self, save=0, img_name_head='GTac_plot'):
        ...
        fig_FA_I.suptitle('Normal Force ' \
                          + self.filename,
                          fontsize=16)
        fig_SFA, axs_SFA = plt.subplots(5, 3,
                                        sharex='all',
                                        sharey='all', constrained_layout=True,
                                        figsize=(15, 15))
        fig_SFA.suptitle('Feature-1 SAII/FAI ' \
                         + self.filename,
                         fontsize=16)
        fig_dSA, axs_dSA = plt.subplots(5, 3,
                                        sharex='all',
                                        sharey='all', constrained_layout=True,
                                        figsize=(15, 15))
        fig_dSA.suptitle('Feature-1 dSA ' \
                         + self.filename,
                         fontsize=16)
        ...
        for f in range(gtac_config.FINGER_NUM):
            for s in range(gtac_config.SEC_NUM):
                ...
                axs_SA_II[f, s].plot(self.sum_value_list[f, s, :]/2, label='FA-I Sum', color='purple', ls='--')  # TODO change the coeficient
                ...
                axs_SFA[f, s].set_title(plot_title)
                axs_SFA[f, s].plot(self.feature_SFA[0, f, s, :], label='Feature-SFA x')
                axs_SFA[f, s].plot(self.feature_SFA[1, f, s, :], label='Feature-SFA y')
                axs_SFA[f, s].plot(self.feature_SFA[2, f, s, :], label='Feature-SFA z')
                # axs_SFA[f, s].legend(loc=0)

                axs_dSA[f, s].set_title(plot_title)
                axs_dSA[f, s].plot(self.feature_dSA[0, f, s, :], label='Feature-dSA x')
                axs_dSA[f, s].plot(self.feature_dSA[1, f, s, :], label='Feature-dSA y')
                axs_dSA[f, s].plot(self.feature_dSA[2, f, s, :], label='Feature-dSA z')
                # axs_dSA[f, s].legend(loc=0)
        ...
        if save:
            ...
            fig_SFA.savefig(img_name_head + '_SFA' + '.svg')
            fig_dSA.savefig(img_name_head + '_dSA' + '.svg')
            ...
            fig_SFA.savefig(img_name_head + '_SFA' + '.png')
            fig_dSA.savefig(img_name_head + '_dSA' + '.png')
    ...
    def get_assigned_sec_wise(self):
        ...
        if self.cutting_points is not None:
            self.data_assigned_sec_wise_pd = self.data_assigned_sec_wise_pd.loc[self.data_assigned_sec_wise_pd['Sample Index'] <= self.cutting_points]
            self.feature_assigned_sec_wise_pd = self.feature_assigned_sec_wise_pd.loc[self.feature_assigned_sec_wise_pd['Sample Index'] <= self.cutting_points]
    ...
    def plot_all_line_in_one(self, save=0, img_name_head='GTac_plot'):
        ...
        FAI_melted = pd.melt(self.data_assigned_sec_wise_pd, id_vars=['finger', 'section', 'milliseconds', 'Sample Index'],
                             value_vars=FAI_cols)
        SAII_melted = pd.melt(self.data_assigned_sec_wise_pd, id_vars=['finger', 'section', 'milliseconds', 'Sample Index'],
                              value_vars=SAII_cols)

        # sns.color_palette("tab10")
        # sns.set_palette("tab10")
        # fig_All_melted, axs_All_melted = plt.subplots(4, 1,
        #                                               sharex='all',
        #                                               sharey=False,
        #                                               constrained_layout=True,
        #                                               figsize=(7, 15))
        #
        # sns.lineplot(x='milliseconds', y='value',
        #              style='finger',
        #              hue='section',
        #              # size='variable',
        #              data=FAI_melted,
        ...
        #              ci=None,
        #              ax=axs_All_melted[0])
        # axs_All_melted[0].set_title('FA-I all in one', fontdict={'fontsize': 8, 'fontweight': 'medium'})
        # sns.lineplot(x='milliseconds', y='value',
        #              # hue='variable',
        #              style='finger',
        #              hue='section',
        #              data=SAII_melted.loc[SAII_melted['variable'] == 'mag_x'],
        #              ci=None,
        #              ax=axs_All_melted[1])
        # axs_All_melted[1].set_title('SA-II all in one', fontdict={'fontsize': 8, 'fontweight': 'medium'})
        # sns.lineplot(x='milliseconds', y='value',
        #              # hue='variable',
        #              style='finger',
        #              hue='section',
        #              data=SAII_melted.loc[SAII_melted['variable'] == 'mag_y'],
        #              ci=None,
        #              ax=axs_All_melted[2])
        # axs_All_melted[2].set_title('SA-II all in one', fontdict={'fontsize': 8, 'fontweight': 'medium'})
        # sns.lineplot(x='milliseconds', y='value',
        #              # hue='variable',
        #              style='finger',
        #              hue='section',
        #              data=SAII_melted.loc[SAII_melted['variable'] == 'mag_z'],
        #              ci=None,
        #              ax=axs_All_melted[3])
        # axs_All_melted[3].set_title('SA-II all in one', fontdict={'fontsize': 8, 'fontweight': 'medium'})
        # for ax in axs_All_melted:
        #     ax.legend([], [], frameon=False)
        # axs_All_melted[0].legend(loc='upper right')
        # axs_All_melted[1].legend(loc='upper right')

        # fig_All_, axs_All_ = plt.subplots(2, 1,
        #                                   sharex='all',
        #                                   sharey=False,
        #                                   constrained_layout=True,
        #                                   figsize=(7, 15))
        # sns.lineplot(data=self.data_pd.iloc[:, gtac_config.MAG_NUM:gtac_config.ALL_GTAC_NUM],
        #              ax=axs_All_[0],
        #              ci=None,
        #              dashes=False,
        #              palette="flare")
        # sns.lineplot(data=self.data_pd.iloc[:, :gtac_config.MAG_NUM],
        #              ax=axs_All_[1],
        #              ci=None,
        #              dashes=False,
        #              palette="flare")
        ...
        # g1 = sns.relplot(x='Sample Index', y='value',
        #                  row='finger',
        #                  col='section',
        #                  hue='variable',
        #                  data=FAI_melted,
        ...
        #                  legend='brief')
        # g1.tight_layout()
        # g.set(xlabel='Time (ms)', ylabel='Value', title='FA-I')
        ...
        # g2 = sns.relplot(x='Sample Index', y='value',
        #                  row='finger',
        #                  col='section',
        #                  hue='variable',
        #                  data=SAII_melted,
        ...
        # # plt.tight_layout()
        # g2.tight_layout()
        # g.set(xlabel='Time (ms)', ylabel='Value', title='SA-II')
...
class gtac_data_analysis(gtac_data):
    ...
    def load_all_data(self, skip_row=0, skip_filename=0):
        if os.path.isfile(self.data_pd_all_filename) and os.path.isfile(self.sec_wise_features_all_filename) and self.load_all:
            ...
            for f in self.filenames:
                ...
                self.sec_wise_features_all = np.concatenate([self.sec_wise_features_all, _gtac_data_set_sec_wise_features],
                                                            axis=3)
            ...
            if self.load_all:
                ...
                print('Saved: shape {} in {}'.format(self.sec_wise_features_all.shape, self.sec_wise_features_all_filename))
        print('ALL dataframes have been loaded, data_pd_all: {}; sec_wise_features_all: {}'.format(self.data_pd_all.shape,
...
if __name__ == '__main__':
    ...
    gtac_data_read.plot_all(save=0, img_name_head='GTac_plot')
    gtac_data_read.plot_tactile_events()
    gtac_data_read.plot_example()
    gtac_data_read.plot_all_line_in_one()
    plt.show()
